...
function detectFundingSignals(html) {
  ...
  if (/(series [a-e]|seed round|raised \$|funding round)/i.test(html)) {
    ...
    signals.push({ label: match ? `Levée de fonds détectée (Series ${match[1].toUpperCase()})` : "Levée de fonds détectée", url: null });
  }
  if (/(Y Combinator|YC \d{2}|backed by)/i.test(html)) {
    signals.push({ label: "Soutenu par un accélérateur (YC ou autre)", url: null });
  }
  return signals;
}
...
// Returns { key, label, url? } objects — url is the actual source page when found
function detectBehavioralSignals(html, baseUrl = "") {
  ...
  if (blogHref) {
    signals.push({ key: "blog", label: "Blog actif → stratégie content marketing en place", url: resolveUrl(blogHref, baseUrl) });
  }

  // Developer focus — extract real docs URL
  const docsHref = extractHref(html, /href=["']([^"']*\/(docs|documentation|api|developers?)[^"']*)["']/i);
  if (docsHref) {
    signals.push({ key: "docs", label: "Documentation/API publique → produit orienté développeurs", url: resolveUrl(docsHref, baseUrl) });
  }
  ...
  if (customersHref) {
    signals.push({ key: "customers", label: "Page clients/cas d'usage → pipeline commercial mature", url: resolveUrl(customersHref, baseUrl) });
  }
  ...
  if (integrationsHref) {
    signals.push({ key: "integrations", label: "Page intégrations → stratégie écosystème développée", url: resolveUrl(integrationsHref, baseUrl) });
  }
  ...
  if (twitterMatch) {
    signals.push({ key: "twitter", label: "Présence Twitter/X → communication publique active", url: twitterMatch[1] });
  } else if (/twitter\.com|x\.com\/[a-zA-Z]/i.test(html)) {
    signals.push({ key: "twitter", label: "Présence Twitter/X → communication publique active", url: null });
  }

  // YouTube — extract actual channel URL
  const youtubeMatch = html.match(/href=["'](https?:\/\/(?:www\.)?youtube\.com\/(?:channel\/|c\/|@)[^"']+)["']/i);
  if (youtubeMatch) {
    signals.push({ key: "youtube", label: "Chaîne YouTube → investissement en contenu vidéo", url: youtubeMatch[1] });
  } else if (/youtube\.com\/(channel|c\/|@)/i.test(html)) {
    signals.push({ key: "youtube", label: "Chaîne YouTube → investissement en contenu vidéo", url: null });
  }
  ...
  if (contactHref) {
    signals.push({ key: "contact-sales", label: "Page 'Contacter les ventes' → cycle de vente assisté", url: resolveUrl(contactHref, baseUrl) });
  }
  ...
  if (discordMatch || slackMatch) {
    ...
    signals.push({ key: "community", label: "Communauté détectée (Slack/Discord) → stratégie PLG", url: communityUrl });
  } else if (/(slack|discord|community)/i.test(html)) {
    signals.push({ key: "community", label: "Communauté détectée (Slack/Discord) → stratégie PLG", url: null });
  }

  // GDPR / EU market
  if (/(cookie consent|gdpr|rgpd)/i.test(html)) {
    signals.push({ key: "gdpr", label: "Conformité GDPR → marché européen ciblé", url: null });
  }
