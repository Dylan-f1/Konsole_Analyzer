...
// Detects infrastructure from HTTP response headers
function detectInfrastructureSignals(headers) {
  const signals = [];
  ...
  if (headers.get("cf-ray") || /cloudflare/i.test(server)) {
    signals.push({ label: "Infrastructure — Cloudflare (CDN/protection DDoS)", url: null });
  }
  if (headers.get("x-vercel-id") || headers.get("x-vercel-cache")) {
    signals.push({ label: "Infrastructure — déployé sur Vercel", url: null });
  }
  if (headers.get("x-amz-request-id") || headers.get("x-amz-id-2")) {
    signals.push({ label: "Infrastructure — hébergé sur AWS S3/CloudFront", url: null });
  }
  if (/netlify/i.test(server) || headers.get("x-nf-request-id")) {
    signals.push({ label: "Infrastructure — déployé sur Netlify", url: null });
  }
  if (/fastly/i.test(server) || headers.get("x-fastly-request-id")) {
    signals.push({ label: "Infrastructure — CDN Fastly", url: null });
  }

  // Web server
  if (/nginx/i.test(server)) {
    signals.push({ label: "Serveur web — nginx", url: null });
  } else if (/apache/i.test(server)) {
    signals.push({ label: "Serveur web — Apache", url: null });
  }

  // Backend language / framework
  if (/express/i.test(poweredBy)) {
    signals.push({ label: "Backend — Node.js / Express", url: null });
  } else if (/php/i.test(poweredBy)) {
    signals.push({ label: "Backend — PHP", url: null });
  } else if (/asp\.net/i.test(poweredBy) || /asp\.net/i.test(server)) {
    signals.push({ label: "Backend — ASP.NET (stack Microsoft)", url: null });
  }

  // Security headers — signal of tech maturity
  const csp = headers.get("content-security-policy");
  const hsts = headers.get("strict-transport-security");
  if (csp && hsts) {
    signals.push({ label: "Sécurité — CSP + HSTS configurés, équipe engineering mature", url: null });
  }
...
export async function scrapeUrl(url) {
  ...
  try {
    ...
    infraSignals = detectInfrastructureSignals(res.headers);
    html = await res.text();
  } catch (err) {
  ...
  return {
    ...
    behavioralSignals: detectBehavioralSignals(html, url),
    infraSignals,
  };
}
